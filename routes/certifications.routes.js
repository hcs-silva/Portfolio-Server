...
router.get("/", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Here are Your Certifications", foundCertifications });
  } catch (error) {
    console.log(error);
...
router.post("/", authenticateUser, async (req, res) => {
  try {
    ...
    const addedCertification =
      await CertificationsModel.create(certificationToAdd);
    res
      .status(201)
      .json({ message: "Certification Added", addedCertification });
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: "Here is the Error", error });
  }
...
router.get("/:certificationId", async (req, res) => {
  ...
  try {
    const oneCertification =
      await CertificationsModel.findById(certificationId);

    res
      .status(200)
      .json({ message: "Here is the Certification", oneCertification });
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: "Here is the Error", error });
  }
